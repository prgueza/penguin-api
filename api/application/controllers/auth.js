...
const { checkValidationCode, generateValidationCode, checkRecoveryCode, generateRecoveryCode } = require('../../helpers/verification/codes')
...
exports.signup = async (req, res) => {
  try {
    ...
    if (password !== confirmPassword) throw new AuthException('SU-31')
    const hashed_pwd = await bcrypt.hash(password, Number(process.env.BCRYPT_SALT_ROUNDS))
    const newUser = new User({ username, email, password: hashed_pwd })
    const user = await newUser.save()
    const token = jwt.sign(user.toJSON(), process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRATION })
    const validationCode = generateValidationCode(user)
    sendValidationMail({ to: user.email, username: user.username, code: validationCode })
    res.status(201).json({
      user,
      token,
      loggedIn: true,
...
exports.validate = async (req, res) => {
  try {
    ...
    if (!checkValidationCode(user, code)) throw await AuthException.init('VC-11')
    const validatedUser = await User.findByIdAndUpdate({ _id: id }, { $set: { validated: true } }, { new: true })
    res.status(200).json({
      user: validatedUser,
      success: true,
      action: 'validate'
...
exports.recover = async (req, res) => {
  try {
    ...
    sendRecoveryMail({ to: user.email, username: user.username, code: recoveryCode })
    res.status(200).json({
      success: true,
      action: 'codegen'
    })
...
exports.reset = async (req, res) => {
  try {
    ...
    if (password !== confirmPassword) throw await AuthException.init('RC-13')
    if (!code) throw await AuthException.init('RC-16')
    const user = await User.findOne({ email })
    if (!checkRecoveryCode(user, code)) throw await AuthException.init('RC-17')
    const hashed_pwd = await bcrypt.hash(password, Number(process.env.BCRYPT_SALT_ROUNDS))
    const updatedUser = await User.findOneAndUpdate({ email }, { $set: { password: hashed_pwd } })
    res.status(200).json({
      user: updatedUser,
      success: true,
      action: 'reset'
